...
const fetchAMFile = async (relativePath, required = true) =>
  fetchRemoteFile(otherExtensionsBaseURL, relativePath, required);
...
const extractLocalAssetPathsFromHTML = (html) => {
  ...
  while ((match = matchAttribute.exec(html)) !== null) {
    ...
    } else if (value.startsWith(`${otherExtensionsBaseURL}/`)) {
      relativePath = value.slice(otherExtensionsBaseURL.length + 1);
    } else {
      continue;
    }
...
const buildOtherOfflineFiles = async () => {
  console.log(`[Other mods] Preparing extension cache from ${otherExtensionsBaseURL}`);

  const tempDirectory = pathUtil.join(import.meta.dirname, '../dist-other-extensions-temp/');
  fs.rmSync(tempDirectory, {
    recursive: true,
    force: true
  });
  console.log('[Other mods] Created temporary output directory');

  const metadataPath = 'data/metadata/other-extensions.json';
  console.log(`${createFetchLogPrefix('Other mods', 'required', 1, 1)} Fetching ${metadataPath}`);
  const metadataBuffer = await fetchAMFile(metadataPath, true);
  const metadata = JSON.parse(metadataBuffer.toString('utf-8'));
  console.log('[Other mods] Parsed metadata');
  ...
  for (const file of requiredFiles) {
    ...
    requiredIndex++;
    console.log(
      `${createFetchLogPrefix('Other mods', 'required', requiredIndex, requiredTotal)} Fetching ${file}`
    );
    try {
      ...
      console.warn(`${createFetchLogPrefix('Other mods', 'required', requiredIndex, requiredTotal)} Failed to fetch ${file}:`, error.message);
    }
  }
  ...
  for (const file of optionalFiles) {
    optionalIndex++;
    console.log(`${createFetchLogPrefix('Other mods', 'optional', optionalIndex)} Fetching ${file}`);
    const data = await fetchAMFile(file, false);
    if (!data) {
      console.log(`${createFetchLogPrefix('Other mods', 'optional', optionalIndex)} Missing ${file}`);
      continue;
    }
    await writeCompressed(tempDirectory, file, data);
    optionalCount++;
  ...
  console.log(
    `Fetched Other mods extensions to ${outputDirectory} (required: ${requiredCount}, optional: ${optionalCount})`
  );
};
