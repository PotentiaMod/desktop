...
const outputDirectory = pathUtil.join(import.meta.dirname, '../dist-gm-extensions/');
const gmExtensionsBaseURL = (
  process.env.GM_EXTENSIONS_BASE_URL || 'https://potentiamod.github.io/extensions'
).replace(/\/+$/, '');
...
const fetchGMFile = async (relativePath, required = true) =>
  fetchRemoteFile(gmExtensionsBaseURL, relativePath, required);
...
const extractLocalAssetPathsFromHTML = (html) => {
  ...
  while ((match = matchAttribute.exec(html)) !== null) {
    ...
    } else if (value.startsWith(`${gmExtensionsBaseURL}/`)) {
      relativePath = value.slice(gmExtensionsBaseURL.length + 1);
    } else {
      continue;
    }
...
const buildGMOfflineFiles = async () => {
  console.log(`[GaiaMod] Preparing extension cache from ${gmExtensionsBaseURL}`);

  const tempDirectory = pathUtil.join(import.meta.dirname, '../dist-gm-extensions-temp/');
  fs.rmSync(tempDirectory, {
    recursive: true,
    force: true
  });
  console.log('[GaiaMod] Created temporary output directory');

  const metadataPath = 'data/metadata/gm-extensions.json';
  console.log(`${createFetchLogPrefix('GaiaMod', 'required', 1, 1)} Fetching ${metadataPath}`);
  const metadataBuffer = await fetchGMFile(metadataPath, true);
  const metadata = JSON.parse(metadataBuffer.toString('utf-8'));
  console.log('[GaiaMod] Parsed metadata');
  ...
  for (const file of requiredFiles) {
    ...
    requiredIndex++;
    console.log(
      `${createFetchLogPrefix('GaiaMod', 'required', requiredIndex, requiredTotal)} Fetching ${file}`
    );
    try {
      const data = await fetchGMFile(file, true);
      await writeCompressed(tempDirectory, file, data);
      requiredCount++;
    } catch (error) {
      console.warn(`${createFetchLogPrefix('GaiaMod', 'required', requiredIndex, requiredTotal)} Failed to fetch ${file}:`, error.message);
    }
  }
  ...
  for (const file of optionalFiles) {
    optionalIndex++;
    console.log(`${createFetchLogPrefix('GaiaMod', 'optional', optionalIndex)} Fetching ${file}`);
    const data = await fetchGMFile(file, false);
    if (!data) {
      console.log(`${createFetchLogPrefix('GaiaMod', 'optional', optionalIndex)} Missing ${file}`);
      continue;
    }
    await writeCompressed(tempDirectory, file, data);
    optionalCount++;
  ...
  console.log(
    `Fetched GaiaMod extensions to ${outputDirectory} (required: ${requiredCount}, optional: ${optionalCount})`
  );
};

try {
  await buildGMOfflineFiles();
} catch (error) {
  console.error(error);
  process.exit(1);
}