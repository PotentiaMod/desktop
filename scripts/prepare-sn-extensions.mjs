...
const fetchSNFile = async (relativePath, required = true) =>
  fetchRemoteFile(snExtensionsBaseURL, relativePath, required);
...
const extractLocalAssetPathsFromHTML = (html) => {
  ...
  while ((match = matchAttribute.exec(html)) !== null) {
    ...
    } else if (value.startsWith(`${snExtensionsBaseURL}/`)) {
      relativePath = value.slice(snExtensionsBaseURL.length + 1);
    } else {
      continue;
    }
...
const buildSNOfflineFiles = async () => {
  console.log(`[Snail IDE] Preparing extension cache from ${snExtensionsBaseURL}`);

  const tempDirectory = pathUtil.join(import.meta.dirname, '../dist-sn-extensions-temp/');
  fs.rmSync(tempDirectory, {
    recursive: true,
    force: true
  });
  console.log('[Snail IDE] Created temporary output directory');

  const metadataPath = 'data/metadata/sn-extensions.json';
  console.log(`${createFetchLogPrefix('Snail IDE', 'required', 1, 1)} Fetching ${metadataPath}`);
  const metadataBuffer = await fetchSNFile(metadataPath, true);
  const metadata = JSON.parse(metadataBuffer.toString('utf-8'));
  console.log('[Snail IDE] Parsed metadata');
  ...
  for (const file of requiredFiles) {
    ...
    requiredIndex++;
    console.log(
      `${createFetchLogPrefix('Snail IDE', 'required', requiredIndex, requiredTotal)} Fetching ${file}`
    );
    try {
      const data = await fetchSNFile(file, true);
      await writeCompressed(tempDirectory, file, data);
      requiredCount++;
    } catch (error) {
      console.warn(`${createFetchLogPrefix('Snail IDE', 'required', requiredIndex, requiredTotal)} Failed to fetch ${file}:`, error.message);
    }
  }
  ...
  for (const file of optionalFiles) {
    optionalIndex++;
    console.log(`${createFetchLogPrefix('Snail IDE', 'optional', optionalIndex)} Fetching ${file}`);
    const data = await fetchSNFile(file, false);
    if (!data) {
      console.log(`${createFetchLogPrefix('Snail IDE', 'optional', optionalIndex)} Missing ${file}`);
      continue;
    }
    await writeCompressed(tempDirectory, file, data);
    optionalCount++;
  ...
  console.log(
    `Fetched Snail IDE extensions to ${outputDirectory} (required: ${requiredCount}, optional: ${optionalCount})`
  );
};

try {
  await buildSNOfflineFiles();
} catch (error) {
  console.error(error);
  process.exit(1);
}