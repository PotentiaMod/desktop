...
const fetchDMFile = async (relativePath, required = true) =>
  fetchRemoteFile(dmExtensionsBaseURL, relativePath, required);
...
const extractLocalAssetPathsFromHTML = (html) => {
  ...
  while ((match = matchAttribute.exec(html)) !== null) {
    ...
    } else if (value.startsWith(`${dmExtensionsBaseURL}/`)) {
      relativePath = value.slice(dmExtensionsBaseURL.length + 1);
    } else {
      continue;
    }
...
const buildDMOfflineFiles = async () => {
  console.log(`[DinosaurMod] Preparing extension cache from ${dmExtensionsBaseURL}`);

  const tempDirectory = pathUtil.join(import.meta.dirname, '../dist-dm-extensions-temp/');
  fs.rmSync(tempDirectory, {
    recursive: true,
    force: true
  });
  console.log('[DinosaurMod] Created temporary output directory');

  const metadataPath = 'data/metadata/dm-extensions.json';
  console.log(`${createFetchLogPrefix('DinosaurMod', 'required', 1, 1)} Fetching ${metadataPath}`);
  const metadataBuffer = await fetchDMFile(metadataPath, true);
  const metadata = JSON.parse(metadataBuffer.toString('utf-8'));
  console.log('[DinosaurMod] Parsed metadata');
  ...
  for (const file of requiredFiles) {
    ...
    requiredIndex++;
    console.log(
      `${createFetchLogPrefix('DinosaurMod', 'required', requiredIndex, requiredTotal)} Fetching ${file}`
    );
    try {
      const data = await fetchDMFile(file, true);
      await writeCompressed(tempDirectory, file, data);
      requiredCount++;
    } catch (error) {
      console.warn(`${createFetchLogPrefix('DinosaurMod', 'required', requiredIndex, requiredTotal)} Failed to fetch ${file}:`, error.message);
    }
  }
  ...
  for (const file of optionalFiles) {
    optionalIndex++;
    console.log(`${createFetchLogPrefix('DinosaurMod', 'optional', optionalIndex)} Fetching ${file}`);
    const data = await fetchDMFile(file, false);
    if (!data) {
      console.log(`${createFetchLogPrefix('DinosaurMod', 'optional', optionalIndex)} Missing ${file}`);
      continue;
    }
    await writeCompressed(tempDirectory, file, data);
    optionalCount++;
  ...
  console.log(
    `Fetched DinosaurMod extensions to ${outputDirectory} (required: ${requiredCount}, optional: ${optionalCount})`
  );
};

try {
  await buildDMOfflineFiles();
} catch (error) {
  console.error(error);
  process.exit(1);
}