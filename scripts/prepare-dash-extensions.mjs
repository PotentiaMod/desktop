...
const fetchDashFile = async (relativePath, required = true) =>
  fetchRemoteFile(dashExtensionsBaseURL, relativePath, required);
...
const extractLocalAssetPathsFromHTML = (html) => {
  ...
  while ((match = matchAttribute.exec(html)) !== null) {
    ...
    } else if (value.startsWith(`${dashExtensionsBaseURL}/`)) {
      relativePath = value.slice(dashExtensionsBaseURL.length + 1);
    } else {
      continue;
    }
...
const buildDashOfflineFiles = async () => {
  console.log(`[Dash] Preparing extension cache from ${dashExtensionsBaseURL}`);

  const tempDirectory = pathUtil.join(import.meta.dirname, '../dist-dash-extensions-temp/');
  ...
  const metadataPath = 'data/metadata/dash-extensions.json';
  console.log(`${createFetchLogPrefix('Dash', 'required', 1, 1)} Fetching ${metadataPath}`);
  const metadataBuffer = await fetchDashFile(metadataPath, true);
  ...
  for (const file of requiredFiles) {
    ...
    try {
      const data = await fetchDashFile(file, true);
  ...
  for (const file of optionalFiles) {
    ...
    const data = await fetchDashFile(file, false);
    if (!data) {
  ...
  console.log(
    `Fetched Dash extensions to ${outputDirectory} (required: ${requiredCount}, optional: ${optionalCount})`
  );
};
