...
const outputDirectory = pathUtil.join(import.meta.dirname, '../dist-am-extensions/');
const amExtensionsBaseURL = (
  process.env.AM_EXTENSIONS_BASE_URL || 'https://potentiamod.github.io/extensions/'
).replace(/\/+$/, '');
...
const fetchAMFile = async (relativePath, required = true) =>
  fetchRemoteFile(amExtensionsBaseURL, relativePath, required);
...
const extractLocalAssetPathsFromHTML = (html) => {
  ...
  while ((match = matchAttribute.exec(html)) !== null) {
    ...
    } else if (value.startsWith(`${amExtensionsBaseURL}/`)) {
      relativePath = value.slice(amExtensionsBaseURL.length + 1);
    } else {
      continue;
    }
...
const buildAMOfflineFiles = async () => {
  console.log(`[AcidMod] Preparing extension cache from ${amExtensionsBaseURL}`);

  const tempDirectory = pathUtil.join(import.meta.dirname, '../dist-am-extensions-temp/');
  fs.rmSync(tempDirectory, {
    recursive: true,
    force: true
  });
  console.log('[AcidMod] Created temporary output directory');

  const metadataPath = 'data/metadata/am-extensions.json';
  console.log(`${createFetchLogPrefix('AcidMod', 'required', 1, 1)} Fetching ${metadataPath}`);
  const metadataBuffer = await fetchAMFile(metadataPath, true);
  const metadata = JSON.parse(metadataBuffer.toString('utf-8'));
  console.log('[AcidMod] Parsed metadata');
  ...
  for (const file of requiredFiles) {
    ...
    requiredIndex++;
    console.log(
      `${createFetchLogPrefix('AcidMod', 'required', requiredIndex, requiredTotal)} Fetching ${file}`
    );
    try {
      const data = await fetchAMFile(file, true);
      await writeCompressed(tempDirectory, file, data);
      requiredCount++;
    } catch (error) {
      console.warn(`${createFetchLogPrefix('AcidMod', 'required', requiredIndex, requiredTotal)} Failed to fetch ${file}:`, error.message);
    }
  }
  ...
  for (const file of optionalFiles) {
    optionalIndex++;
    console.log(`${createFetchLogPrefix('AcidMod', 'optional', optionalIndex)} Fetching ${file}`);
    const data = await fetchAMFile(file, false);
    if (!data) {
      console.log(`${createFetchLogPrefix('AcidMod', 'optional', optionalIndex)} Missing ${file}`);
      continue;
    }
    await writeCompressed(tempDirectory, file, data);
    optionalCount++;
  ...
  console.log(
    `Fetched AcidMod extensions to ${outputDirectory} (required: ${requiredCount}, optional: ${optionalCount})`
  );
};

try {
  await buildAMOfflineFiles();
} catch (error) {
  console.error(error);
  process.exit(1);
}