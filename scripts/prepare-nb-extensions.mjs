...
const outputDirectory = pathUtil.join(import.meta.dirname, '../dist-nb-extensions/');
const nbExtensionsBaseURL = (
  process.env.NB_EXTENSIONS_BASE_URL || 'https://potentiamod.github.io/extensions'
).replace(/\/+$/, '');
...
const fetchNBFile = async (relativePath, required = true) =>
  fetchRemoteFile(nbExtensionsBaseURL, relativePath, required);
...
const extractLocalAssetPathsFromHTML = (html) => {
  ...
  while ((match = matchAttribute.exec(html)) !== null) {
    ...
    } else if (value.startsWith(`${nbExtensionsBaseURL}/`)) {
      relativePath = value.slice(nbExtensionsBaseURL.length + 1);
    } else {
      continue;
    }
...
const buildNBOfflineFiles = async () => {
  console.log(`[NitroBolt] Preparing extension cache from ${nbExtensionsBaseURL}`);

  const tempDirectory = pathUtil.join(import.meta.dirname, '../dist-nb-extensions-temp/');
  fs.rmSync(tempDirectory, {
    recursive: true,
    force: true
  });
  console.log('[NitroBolt] Created temporary output directory');

  const metadataPath = 'data/metadata/nb-extensions.json';
  console.log(`${createFetchLogPrefix('NitroBolt', 'required', 1, 1)} Fetching ${metadataPath}`);
  const metadataBuffer = await fetchNBFile(metadataPath, true);
  const metadata = JSON.parse(metadataBuffer.toString('utf-8'));
  console.log('[NitroBolt] Parsed metadata');
  ...
  for (const file of requiredFiles) {
    ...
    requiredIndex++;
    console.log(
      `${createFetchLogPrefix('NitroBolt', 'required', requiredIndex, requiredTotal)} Fetching ${file}`
    );
    try {
      const data = await fetchNBFile(file, true);
      await writeCompressed(tempDirectory, file, data);
      requiredCount++;
    } catch (error) {
      console.warn(`${createFetchLogPrefix('NitroBolt', 'required', requiredIndex, requiredTotal)} Failed to fetch ${file}:`, error.message);
    }
  }
  ...
  for (const file of optionalFiles) {
    optionalIndex++;
    console.log(`${createFetchLogPrefix('NitroBolt', 'optional', optionalIndex)} Fetching ${file}`);
    const data = await fetchNBFile(file, false);
    if (!data) {
      console.log(`${createFetchLogPrefix('NitroBolt', 'optional', optionalIndex)} Missing ${file}`);
      continue;
    }
    await writeCompressed(tempDirectory, file, data);
    optionalCount++;
  ...
  console.log(
    `Fetched NitroBolt extensions to ${outputDirectory} (required: ${requiredCount}, optional: ${optionalCount})`
  );
};

try {
  await buildNBOfflineFiles();
} catch (error) {
  console.error(error);
  process.exit(1);
}