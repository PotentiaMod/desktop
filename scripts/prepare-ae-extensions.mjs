...
const outputDirectory = pathUtil.join(import.meta.dirname, '../dist-ae-extensions/');
const aeExtensionsBaseURL = (
  process.env.AE_EXTENSIONS_BASE_URL || 'https://potentiamod.github.io/extensions/'
).replace(/\/+$/, '');
...
const fetchAEFile = async (relativePath, required = true) =>
  fetchRemoteFile(aeExtensionsBaseURL, relativePath, required);
...
const extractLocalAssetPathsFromHTML = (html) => {
  ...
  while ((match = matchAttribute.exec(html)) !== null) {
    ...
    } else if (value.startsWith(`${aeExtensionsBaseURL}/`)) {
      relativePath = value.slice(aeExtensionsBaseURL.length + 1);
    } else {
      continue;
    }
...
const buildAEOfflineFiles = async () => {
  console.log(`[Astra Editor] Preparing extension cache from ${aeExtensionsBaseURL}`);

  const tempDirectory = pathUtil.join(import.meta.dirname, '../dist-ae-extensions-temp/');
  fs.rmSync(tempDirectory, {
    recursive: true,
    force: true
  });
  console.log('[Astra Editor] Created temporary output directory');

  const metadataPath = 'data/metadata/ae-extensions.json';
  console.log(`${createFetchLogPrefix('Astra Editor', 'required', 1, 1)} Fetching ${metadataPath}`);
  const metadataBuffer = await fetchAEFile(metadataPath, true);
  const metadata = JSON.parse(metadataBuffer.toString('utf-8'));
  console.log('[Astra Editor] Parsed metadata');
  ...
  for (const file of requiredFiles) {
    ...
    requiredIndex++;
    console.log(
      `${createFetchLogPrefix('Astra Editor', 'required', requiredIndex, requiredTotal)} Fetching ${file}`
    );
    try {
      ...
      console.warn(`${createFetchLogPrefix('Astra Editor', 'required', requiredIndex, requiredTotal)} Failed to fetch ${file}:`, error.message);
    }
  }
  ...
  for (const file of optionalFiles) {
    optionalIndex++;
    console.log(`${createFetchLogPrefix('Astra Editor', 'optional', optionalIndex)} Fetching ${file}`);
    const data = await fetchAEFile(file, false);
    if (!data) {
      console.log(`${createFetchLogPrefix('Astra Editor', 'optional', optionalIndex)} Missing ${file}`);
      continue;
    }
    await writeCompressed(tempDirectory, file, data);
    optionalCount++;
  ...
  console.log(
    `Fetched Astra Editor extensions to ${outputDirectory} (required: ${requiredCount}, optional: ${optionalCount})`
  );
};

try {
  await buildAEOfflineFiles();
} catch (error) {
  console.error(error);
  process.exit(1);
}